...
// Remove OpenAI related constants
const SPOONACULAR_API_KEY = 'ccfb684dadcc4394a8d2c89b3421313b';
...
function RecipeRecommendations({ userProfile }) {
  ...
  // Update the handleGetRecipes function
  const handleGetRecipes = async () => {
    ...
    try {
      ...
      // Get recipes from both Spoonacular and IC LLM
      const [spoonacularRecipes, llmRecipe] = await Promise.allSettled([
        ...
        (async () => {
          ...
          const findResponse = await fetch(`${SPOONACULAR_BASE_URL}/findByIngredients?${findParams}`);
          if (!findResponse.ok) {
          ...
          const detailedRecipes = await Promise.all(
            foundRecipes.map(async (recipe) => {
              ...
              const infoResponse = await fetch(`${SPOONACULAR_BASE_URL}/${recipe.id}/information?${infoParams}`);
              if (!infoResponse.ok) {
        ...
        // IC LLM Processing through backend canister
        (async () => {
          const response = await window.canister.backend.getRecipeRecommendations(
            ingredients.map(ing => ({
              id: ing.id,
              name: ing.name,
      ...
      // Handle Spoonacular results
      if (spoonacularRecipes.status === 'fulfilled' && spoonacularRecipes.value?.length > 0) {
        const transformedSpoonacularRecipes = spoonacularRecipes.value.map(recipe => ({
          name: recipe.title,
          description: recipe.summary,
          cookingTime: `${recipe.readyInMinutes} minutes`,
          difficulty: recipe.dishTypes?.[0] || 'Medium',
          servings: recipe.servings,
          calories: `${Math.round(recipe.nutrition?.nutrients?.find(n => n.name === 'Calories')?.amount || 0)} per serving`,
          ...
          tips: [
            ...
            'Follow temperature guidelines carefully'
          ],
          nutritionalInfo: {
            ...
            fiber: `${Math.round(recipe.nutrition?.nutrients?.find(n => n.name === 'Fiber')?.amount || 0)}g`,
            sodium: `${Math.round(recipe.nutrition?.nutrients?.find(n => n.name === 'Sodium')?.amount || 0)}mg`
          },
          imageUrl: recipe.image,
          source: 'Spoonacular'
        }));
        combinedRecipes = [...transformedSpoonacularRecipes];
      }

      // Handle IC LLM results
      if (llmRecipe.status === 'fulfilled' && llmRecipe.value) {
        // Parse the LLM response text into recipe format
        const llmRecipeObject = parseLLMResponse(llmRecipe.value);
        if (llmRecipeObject) {
      ...
      if (combinedRecipes.length === 0) {
        throw new Error('No recipes found. Please try different ingredients or preferences.');
      }
  ...
  // Add helper function to parse LLM response
  const parseLLMResponse = (llmResponse) => {
    // This is a simple parser - you might need to adjust based on actual LLM output format
    try {
      // Split the response into sections
      const sections = llmResponse.split('\n\n');
      
      // Extract basic information
      const name = sections[0].replace('Recipe name: ', '').trim();
      const description = sections[1].replace('Description: ', '').trim();
      const cookingTime = sections[2].replace('Cooking time: ', '').trim();
      const difficulty = sections[3].replace('Difficulty level: ', '').trim();
      const servings = sections[4].replace('Servings: ', '').trim();
      const calories = sections[5].replace('Calories: ', '').trim();
      
      // Parse ingredients
      const ingredientsSection = sections.find(s => s.includes('Ingredients:'));
      ...
      // Parse instructions
      const instructionsSection = sections.find(s => s.includes('Instructions:'));
      ...
      // Parse tips
      const tipsSection = sections.find(s => s.includes('Tips:'));
      ...
      const nutritionSection = sections.find(s => s.includes('Nutritional information:'));
      const nutritionalInfo = {
        protein: nutritionSection.match(/protein: ([\d.]+g)/i)?.[1] || '0g',
        carbs: nutritionSection.match(/carbs: ([\d.]+g)/i)?.[1] || '0g',
        fat: nutritionSection.match(/fat: ([\d.]+g)/i)?.[1] || '0g',
        fiber: nutritionSection.match(/fiber: ([\d.]+g)/i)?.[1] || '0g',
        sodium: nutritionSection.match(/sodium: ([\d.]+mg)/i)?.[1] || '0mg'
      };
      ...
      return {
        ...
        tips,
        nutritionalInfo,
        imageUrl: `https://source.unsplash.com/featured/?${encodeURIComponent(name.split(' ').join(','))},food,dish`
      };
    } catch (error) {
